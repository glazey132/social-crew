...
def test_orchestrator_run_daily_persists_pending_approvals(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("pipeline.orchestrator.download_video", lambda url, output_dir, dry_run=True: tmp_path / "src_1.mp4")
    monkeypatch.setattr(
        "pipeline.orchestrator.transcribe_video",
        lambda _: [ClipSegment(candidate_id="src_1", start_sec=0, end_sec=30, hook_text="hook", confidence=0.95)],
    )
    monkeypatch.setattr(
        "pipeline.orchestrator.render_clip",
        lambda source_path, output_dir, segment, dry_run=True: RenderedClip(
            clip_id="clip_1",
...
def test_orchestrator_filters_processed_sources(tmp_path: Path):
    config = PipelineConfig(
        llm_model="ollama/test",
        llm_base_url="http://localhost:11434",
        workspace_dir=tmp_path,
